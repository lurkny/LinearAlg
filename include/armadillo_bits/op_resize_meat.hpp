...
op_resize::apply(Mat<typename T1::elem_type>& out, const Op<T1,op_resize>& in)
  ...
  if(&out == &A)
    {
    op_resize::apply_inplace(out, new_n_rows, new_n_cols);
    }
  else
    {
    op_resize::apply_noalias(out, A, new_n_rows, new_n_cols);
    }
  }
...
inline
void
op_resize::apply_inplace(Mat<eT>& A, const uword new_n_rows, const uword new_n_cols)
  {
  arma_extra_debug_sigprint();
  ...
  op_resize::apply_noalias(B, A, new_n_rows, new_n_cols);
...
inline
void
op_resize::apply_noalias(Mat<eT>& out, const Mat<eT>& A, const uword new_n_rows, const uword new_n_cols)
  {
  arma_extra_debug_sigprint();
