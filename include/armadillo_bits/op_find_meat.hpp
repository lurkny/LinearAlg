...
inline
void
op_find_inf::apply(Mat<uword>& out, const mtOp<uword, T1, op_find_inf>& X)
  {
  arma_extra_debug_sigprint();
  ...
  if(Proxy<T1>::use_at == false)
    ...
    for(uword i=0; i<n_elem; ++i)
      {
      if( arma_isinf(Pea[i]) )  { indices_mem[count] = i; count++; }
      }
    }
  else
    {
    ...
    for(uword row=0; row<n_rows; ++row)
      {
      if( arma_isinf(P.at(row,col)) )  { indices_mem[count] = i; count++; }
